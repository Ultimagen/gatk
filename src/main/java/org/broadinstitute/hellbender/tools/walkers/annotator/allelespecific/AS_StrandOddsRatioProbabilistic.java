...
/**
 * Allele-specific strand bias estimated by the Symmetric Odds Ratio (Probabilistic) test
 *
...
@DocumentedFeature(groupName= HelpConstants.DOC_CAT_ANNOTATORS, groupSummary=HelpConstants.DOC_CAT_ANNOTATORS_SUMMARY,
        summary="Allele-specific strand bias estimated by the symmetric odds " +
                "ratio test taking care of probabilities(AS_SOR)")
public class AS_StrandOddsRatioProbabilistic extends AS_StrandBiasTestProbabilistic implements AS_StandardAnnotation {
    public List<String> getKeyNames() {
        return Collections.singletonList(GATKVCFConstants.AS_STRAND_ODDS_RATIO_PROB_KEY);
    }
    ...
    protected Map<Allele,Double> calculateReducedData(AlleleSpecificAnnotationData<List<Double>> combinedData) {
        final Map<Allele,Double> annotationMap = new HashMap<>();
        final Map<Allele, List<Double>> perAlleleData = combinedData.getAttributeMap();
        final List<Double> refStrandCounts = perAlleleData.get(combinedData.getRefAllele());
        for (final Allele a : perAlleleData.keySet()) {
            List<Double> altStrandCounts = perAlleleData.get(a);
            double[][] refAltTable = new double[][] {new double[]{refStrandCounts.get(FORWARD),refStrandCounts.get(REVERSE)},
                    new double[]{altStrandCounts.get(FORWARD),altStrandCounts.get(REVERSE)}};
