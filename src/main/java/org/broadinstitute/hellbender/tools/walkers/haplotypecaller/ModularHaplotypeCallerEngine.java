...
public class ModularHaplotypeCallerEngine extends RampedHaplotypeCallerEngine {

    private final Logger logger = LogManager.getLogger(ModularHaplotypeCallerEngine.class);
    ...
    public ModularHaplotypeCallerEngine(final HaplotypeCallerArgumentCollection hcArgs, final AssemblyRegionArgumentCollection assemblyRegionArgs, final boolean createBamOutIndex,
    ...
    public List<VariantContext> callRegion(final AssemblyRegion region, final FeatureContext features, final ReferenceContext referenceContext) {
        ...
        RampUtils.logReads(hcArgs.rampsDebugReads, "callRegion entered: " + region, region.getReads());
    ...
    private void assemble(final CallRegionContext context) throws IOException {
        ...
        if ( postAssemblerOnRamp != null ) {
            ...
            RampUtils.logReads(hcArgs.rampsDebugReads, "onramp: reads before trimming", context.assemblyResult.getRegionForGenotyping().getReads());

            RampUtils.logReads(hcArgs.rampsDebugReads, "onramp: BEFORE untrimmedAssemblyResult reads", context.region.getReads());
            ...
            RampUtils.logReads(hcArgs.rampsDebugReads, "onramp: AFTER untrimmedAssemblyResult reads", context.region.getReads());
            ...
            context.haplotypeCollapsing = context.assemblyResult.getHaplotypeCollapsing();
            RampUtils.logReads(hcArgs.rampsDebugReads, "onramp: reads after trimming", context.assemblyResult.getRegionForGenotyping().getReads());

            if ( hcArgs.rampsDebugPostAssemblerOn) {
        ...
        RampUtils.logReads(hcArgs.rampsDebugReads, "BEFORE untrimmedAssemblyResult reads", context.region.getReads());
        final AssemblyResultSet untrimmedAssemblyResult = AssemblyBasedCallerUtils.assembleReads(context.region, context.givenAlleles, hcArgs, readsHeader, samplesList, logger, referenceReader, assemblyEngine, aligner,
                !hcArgs.doNotCorrectOverlappingBaseQualities, hcArgs.fbargs, postFilterOnRamp != null);
        RampUtils.logReads(hcArgs.rampsDebugReads, "AFTER untrimmedAssemblyResult reads", context.region.getReads());
        ...
        RampUtils.logReads(hcArgs.rampsDebugReads, "untrimmedAssemblyResult reads", untrimmedAssemblyResult.getRegionForGenotyping().getReads());
        ...
        RampUtils.logReads(hcArgs.rampsDebugReads, "assemblyResult final reads", context.assemblyResult.getRegionForGenotyping().getReads());

        if ( hcArgs.rampsDebugPostAssemblerOn && debugContext != null ) {
    ...
    private void filter(final CallRegionContext context) throws IOException {
        ...
        for ( int sampleIndex = 0; sampleIndex < context.readLikelihoods.samples().size() ; sampleIndex++ )
            RampUtils.logReads(hcArgs.rampsDebugReads, "filter (END): " + sampleIndex, context.readLikelihoods.sampleEvidence(sampleIndex));
    }
